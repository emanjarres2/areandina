...
public class ModificarFacultades extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action1.equals(editar)) {
            try {
                String consulta = "UPDATE facultad SET Name_faculty='" + nombre + "', Id_campus='" + id_campus + "' WHERE Id_faculty=" + id;
        ...
        if (action2.equals(borrar)) {
            try {
                String consulta = "DELETE FROM facultad WHERE Id_faculty=" + id;
                pst = (PreparedStatement) conexion.getConexion().prepareStatement(consulta);
                pst.executeUpdate();

                if (pst.executeUpdate() == 0) {
                    response.sendRedirect("administrador/ModificarFacultad.jsp");
                } else {
